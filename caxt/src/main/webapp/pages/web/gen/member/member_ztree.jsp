...
<script type="text/javascript">
	...
	function initTree(family_id){
		var treeNodes = '';
		$.ajax({
			url : "${pageContext.request.contextPath}/gen/member/findGenerationList.do?family_id="+family_id,
			data : {},
			type : 'GET',
			dataType : 'json',
			success : function(res) {
				if (res.success) {
					treeNodes += "[{id:'0',parent_id:'parent',name:'世代信息'},";
					$.each(res.data,function(i,n){
						treeNodes += "{";
						treeNodes += "id:'"+n.id+"'";
						treeNodes += ",parent_id:'"+n.parent_id+"'";
						treeNodes += ",name:'"+n.name+"'";
						treeNodes += ",dic_cascade:'"+n.dic_cascade+"'";
						treeNodes += "},";
					});
	...
	function expandFirst(){
		...
		$("#dic_cascade").val(nodes[0].dic_cascade);
	...
	function expandDefault(tree_id){
		...
		$("#dic_cascade").val(node.dic_cascade);
	...
	function zTreeOnClick(event, treeId, treeNode) {
		...
		$("#dic_cascade").val(treeNode.dic_cascade);
