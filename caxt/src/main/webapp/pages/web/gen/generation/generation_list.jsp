...
<script>
    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,url: '${pageContext.request.contextPath}/gen/generation/findListPage.do?auth_type=${pd.auth_type}&auth_organize_ids=${pd.auth_organize_ids}'
                ,cols: [[
        ...
        form.on('switch(statusCheck)', function(obj){
            ...
            $.get("${pageContext.request.contextPath}/gen/generation/updateStatus.do?id="+this.value+"&status="+status,null,function(res){
                console.log(res);
                if (res.success) {
                    layer.tips("状态修改成功。");
                }else{
        ...
        $('#dels').on('click',function (){
            ...
            if(data.length>0){
                layer.confirm('真的要删除数据么', function(index){
                    ...
                    $.get("${pageContext.request.contextPath}/gen/generation/del.do?ids="+ids,null,function(res){
                        if (res.success) {
        ...
        table.on('tool(generation)', function(obj){
            ...
            if(obj.event === 'detail'){
                ...
                layer.confirm('真的要删除数据么', function(index){
                    $.get("${pageContext.request.contextPath}/gen/generation/del.do?ids="+data.id,null,function(res){
                        if (res.success) {
                            layer.msg("数据删除成功。", {time: 2000});
                            location.replace(location.href);
                        }else{
    ...
    function add(){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/gen/generation/toAdd.do',
            area: ['800px', '500px'],
            end: function () {
                location.reload();
            }
    ...
    function edit(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/gen/generation/toUpdate.do?id='+id,
            area: ['800px', '500px'],
            end: function () {
                location.reload();
            }
    ...
    function info(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/gen/generation/findInfo.do?id='+id,
            area: ['800px', '500px']
        });
    }
