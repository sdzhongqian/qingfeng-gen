...
<div class="admin-main">
    <blockquote id="search" class="layui-elem-quote" style="position: relative;">
        <div class="layui-form-item" style="margin-bottom: 0;">
            <div class="layui-inline">
                <label class="layui-form-lab" style="width: auto">家族名称</label>
                <input type="text" name="name" id="name" style="width:190px;height: 32px;display: inline" placeholder="请输入家族名称" class="layui-input">
            </div>
            <div class="layui-inline">
                <label class="layui-form-lab" style="width: auto">姓氏</label>
                <input type="text" name="first_name" id="first_name" style="width:190px;height: 32px;display: inline" placeholder="请输入姓氏" class="layui-input">
            </div>
            <div class="layui-inline">
    ...
    <div class="layui-field-box layui-form" style="padding: 0;">
        <table id="LAY_table_family" class="layui-table admin-table" lay-filter="family">
        </table>
    </div>
...
<script>
    ...
    layui.use('table', function(){
        ...
        //方法级渲染
        table.render({
            elem: '#LAY_table_family'
            ,url: '${pageContext.request.contextPath}/gen/family/findListPage.do?auth_type=${pd.auth_type}&auth_organize_ids=${pd.auth_organize_ids}'
                ,cols: [[
                {checkbox: true, fixed: true}
                ,{field:'name', title: '家族名称', width:120, sort: true, fixed: true}
                ,{field:'first_name', title: '姓氏'}
        ...
        form.on('switch(statusCheck)', function(obj){
            ...
            $.get("${pageContext.request.contextPath}/gen/family/updateStatus.do?id="+this.value+"&status="+status,null,function(res){
                console.log(res);
                if (res.success) {
                    layer.tips("状态修改成功。");
                }else{
        ...
        $('#edits').on('click',function (){
            var checkStatus = table.checkStatus('table_family')
        ...
        $('#dels').on('click',function (){
            ...
            if(data.length>0){
                layer.confirm('真的要删除数据么', function(index){
                    ...
                    $.get("${pageContext.request.contextPath}/gen/family/del.do?ids="+ids,null,function(res){
                        if (res.success) {
        ...
        table.on('tool(family)', function(obj){
            ...
            if(obj.event === 'detail'){
                ...
                layer.confirm('真的要删除数据么', function(index){
                    $.get("${pageContext.request.contextPath}/gen/family/del.do?ids="+data.id,null,function(res){
                        if (res.success) {
                            layer.msg("数据删除成功。", {time: 2000});
                            location.replace(location.href);
                        }else{
    ...
    function add(){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/gen/family/toAdd.do',
            area: ['800px', '500px'],
            end: function () {
                location.reload();
            }
    ...
    function edit(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/gen/family/toUpdate.do?id='+id,
            area: ['800px', '500px'],
            end: function () {
                location.reload();
            }
    ...
    function info(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/gen/family/findInfo.do?id='+id,
            area: ['800px', '500px']
        });
    }
