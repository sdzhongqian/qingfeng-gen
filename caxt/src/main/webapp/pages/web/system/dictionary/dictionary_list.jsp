...
<div class="x-nav">
    ...
    <span class="layui-btn layui-btn-sm" id="current_dic"></span>
...
<div class="admin-main">
    ...
    <blockquote id="search" class="layui-elem-quote">
        ...
        <input type="text" name="name" id="name" style="width: 240px;height: 32px;display: inline" placeholder="请输入字典名称" class="layui-input">
        <label class="layui-form-lab" style="width: auto">字典简称</label>
        <input type="text" name="short_name" id="short_name" style="width: 240px;height: 32px;display: inline" placeholder="请输入字典简称" class="layui-input">
        <a href="javascript:;" class="layui-btn layui-btn-sm" data-type="reload">
            <i class="layui-icon">&#xe615;</i> 搜索
        </a>
    </blockquote>
    <div class="layui-field-box layui-form" style="padding: 0;">
        <table id="LAY_table_dictionary" class="layui-table admin-table" lay-filter="dictionary">
        </table>
    </div>
...
<script>
    ...
    layui.use('table', function(){
        ...
        //方法级渲染
        table.render({
            elem: '#LAY_table_dictionary'
            ,url: '${pageContext.request.contextPath}/system/dictionary/findListPage.do?auth_type=${pd.auth_type}&auth_organize_ids=${pd.auth_organize_ids}'
                ,cols: [[
                {checkbox: true, fixed: true}
                ,{field:'id', title: '编号', width:80, sort: true, fixed: true}
                ,{field:'name', title: '字典名称', width:120}
                ,{field:'short_name', title: '字典简称', width:120}
        ...
        $('#edits').on('click',function (){
            var checkStatus = table.checkStatus('table_dictionary')
        ...
        $('#dels').on('click',function (){
            ...
            if(data.length>0){
                layer.confirm('删除数据会同步删除下级数据，是否进行删除操作？', function(index){
                    ...
                    $.get("${pageContext.request.contextPath}/system/dictionary/del.do?ids="+ids,null,function(res){
                        if (res.success) {
        ...
        table.on('tool(dictionary)', function(obj){
            ...
            if(obj.event === 'detail'){
                ...
                layer.confirm('删除数据会同步删除下级数据，是否进行删除操作？', function(index){
                    $.get("${pageContext.request.contextPath}/system/dictionary/del.do?ids="+data.id,null,function(res){
                        if (res.success) {
                            layer.msg("数据删除成功。", {time: 2000});
                            location.replace(location.href);
                        }else{
    ...
    function add(){
        ...
        if(parent_id==''||parent_id==null){
            ...
            parent.layer.open({
                ...
                maxmin: true,
                type: 2,
                content: '${pageContext.request.contextPath}/system/dictionary/toAdd.do?parent_id='+parent_id+'&name='+name+'&dic_cascade='+dic_cascade,
                area: ['800px', '500px'],
                end: function () {
                    location.reload();
                }
    ...
    function addMore(){
        ...
        if(parent_id==''||parent_id==null){
            ...
            parent.layer.open({
                ...
                maxmin: true,
                type: 2,
                content: '${pageContext.request.contextPath}/system/dictionary/toAddMore.do?parent_id='+parent_id+'&name='+name+'&dic_cascade='+dic_cascade,
                area: ['1000px', '500px'],
                end: function () {
                    location.reload();
                }
    ...
    function edit(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/system/dictionary/toUpdate.do?id='+id,
            area: ['800px', '500px'],
            end: function () {
                location.reload();
            }
    ...
    function info(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/system/dictionary/findInfo.do?id='+id,
            area: ['800px', '500px']
        });
    }
