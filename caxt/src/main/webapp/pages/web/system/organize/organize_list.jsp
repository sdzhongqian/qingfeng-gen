...
<script>
    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,url: '${pageContext.request.contextPath}/system/organize/findListPage.do?auth_type=${pd.auth_type}&auth_organize_ids=${pd.auth_organize_ids}'
                ,cols: [[
        ...
        $('#dels').on('click',function (){
            ...
            if(data.length>0){
                layer.confirm('删除数据会同步删除下级数据，是否进行删除操作？', function(index){
                    ...
                    $.get("${pageContext.request.contextPath}/system/organize/del.do?ids="+ids,null,function(res){
                        if (res.success) {
        ...
        table.on('tool(organize)', function(obj){
            ...
            if(obj.event === 'detail'){
                ...
                layer.confirm('删除数据会同步删除下级数据，是否进行删除操作？', function(index){
                    $.get("${pageContext.request.contextPath}/system/organize/del.do?ids="+data.id,null,function(res){
                        if (res.success) {
                            layer.msg("数据删除成功。", {time: 2000});
                            location.replace(location.href);
                        }else{
    ...
    function add(){
        ...
        if(parent_id==''||parent_id==null){
            ...
            parent.layer.open({
                ...
                maxmin: true,
                type: 2,
                content: '${pageContext.request.contextPath}/system/organize/toAdd.do?parent_id='+parent_id+'&name='+name+'&level='+level+'&org_cascade='+org_cascade+'&parent_code='+parent_code,
                area: ['800px', '500px'],
                end: function () {
                    location.reload();
                }
    ...
    function addMore(){
        ...
        if(parent_id==''||parent_id==null){
            ...
            parent.layer.open({
                ...
                maxmin: true,
                type: 2,
                content: '${pageContext.request.contextPath}/system/organize/toAddMore.do?parent_id='+parent_id+'&name='+name+'&level='+level+'&org_cascade='+org_cascade+'&parent_code='+parent_code,
                area: ['1000px', '500px'],
                end: function () {
                    location.reload();
                }
    ...
    function edit(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/system/organize/toUpdate.do?id='+id,
            area: ['800px', '500px'],
            end: function () {
                location.reload();
            }
    ...
    function info(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/system/organize/findInfo.do?id='+id,
            area: ['800px', '500px']
        });
    }
