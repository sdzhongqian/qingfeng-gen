...
<div id="jpstr" style="margin-top: 10px;">
    ...
    <button class="layui-btn layui-btn-sm" onclick="toExport('org','${p.id}');">导出</button>
</div>
<form class="layui-form" action="${pageContext.request.contextPath}/system/organize/update.do" id="form">
    <table width="90%">
        <tr>
            <td width="20%" align="right"><label class="layui-form-label">父级名称：</label></td>
            <td colspan="3">
                <c:if test="${p.parent_name==null}"><input type="text" readonly autocomplete="off" placeholder="请输入父级名称" value="组织信息" class="layui-input"></c:if>
...
<script>
    layui.use(['form'], function(){
        ...
        form.on('submit(submit_form)', function(data){
            $.post("${pageContext.request.contextPath}/system/organize/update.do",data.field,function(res){
                if (res.success) {
